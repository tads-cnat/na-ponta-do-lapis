...
def index(request):
    """
    View principal que exibe as contas do usuário logado ou contas da sessão.
    Aceita usuários autenticados (banco) e anônimos (sessão).
    """
    try:
        if request.user.is_authenticated:
            # Usuário autenticado: obter contas do banco
            contas = ContaService.obter_contas_usuario(request.user)
        else:
            # Usuário anônimo: obter contas da sessão
            contas = ContaService.obter_contas_sessao(request)
        ...
        if request.user.is_authenticated:
            ...
            # Para anônimos, manter na mesma página
            return render(request, 'contas/index.html', {'contas': [], 'tem_contas': False})

def visualizar_conta(request, conta_id):
    """
    View para visualizar uma conta específica.
    Funciona para contas do banco (autenticado) ou sessão (anônimo).
    """
    try:
        if request.user.is_authenticated:
            # Usuário autenticado: obter conta do banco
            ...
            # Usuário anônimo: obter conta da sessão
            contas = ContaService.obter_contas_sessao(request)
            ...
            if not conta_encontrada:
                messages.error(request, 'Conta não encontrada na sessão.')
                raise Http404('Conta não encontrada na sessão.')
            
            contexto = {'conta': conta_encontrada}
            return render(request, 'contas/visualizar_conta.html', contexto)
    except Http404:
        raise
    except Exception as e:
...
def add_conta(request):
    ...
    Salva no banco se autenticado, ou na sessão se anônimo.
    """
    try:
        if request.method == 'POST':
            ...
            if request.user.is_authenticated:
                ...
                # Usuário anônimo: salvar na sessão
                conta = ContaService.salvar_conta_sessao(request, nome, saldo, tipo)
                messages.success(request, f'Conta "{conta["nome"]}" criada na sessão com sucesso!')
...
def editar_conta(request, conta_id):
    ...
    try:
        if request.user.is_authenticated:
            ...
            # Usuário anônimo: editar conta da sessão
            contas = ContaService.obter_contas_sessao(request)
            conta_encontrada = None
            for conta in contas:
                if conta["id"] == int(conta_id):
            ...
            if request.method == 'POST':
                ...
                conta = ContaService.editar_conta_sessao(request, int(conta_id), nome, saldo, tipo)
                
                if conta:
                    messages.success(request, f'Conta "{conta["nome"]}" atualizada na sessão com sucesso!')
                    return redirect('contas:index')
...
def excluir_conta(request, conta_id):
    ...
    try:
        if request.user.is_authenticated:
            # Usuário autenticado: excluir conta do banco
            conta = ContaService.obter_conta_por_id(conta_id)
            ...
            # Usuário anônimo: excluir conta da sessão
            try:
                ContaService.excluir_conta_sessao(request, int(conta_id))
                return JsonResponse({
                    'sucesso': True,
                    'mensagem': 'Conta excluída da sessão com sucesso!'
                })
            except ValidationError as e:
                return JsonResponse({'sucesso': False, 'erro': str(e.message)}, status=404)
    except Exception as e:
...
def obter_conta_json(request, conta_id):
    ...
    Suporta contas do banco e da sessão.
    """
    try:
        if request.user.is_authenticated:
            # Usuário autenticado: obter conta do banco
            ...
            if not conta_encontrada:
                return JsonResponse({'sucesso': False, 'erro': 'Conta não encontrada na sessão'}, status=404)
            
            return JsonResponse({
                'id': conta_encontrada["id"],
                'nome': conta_encontrada["nome"],
                'saldo': conta_encontrada["saldo"],
                'tipo': conta_encontrada["tipo"],
                'sucesso': True
            })
    except Exception as e:
        return JsonResponse({'sucesso': False, 'erro': str(e)}, status=500)
...
def obter_transacoes_conta(request, conta_id):
    ...
    Para anônimos, retorna lista vazia (sem transações no banco).
    """
    try:
        if request.user.is_authenticated:
            # Usuário autenticado: obter transações do banco
            conta = ContaService.obter_conta_por_id(conta_id)
            ...
            # Usuário anônimo: obter conta da sessão (sem transações)
            contas = ContaService.obter_contas_sessao(request)
            conta_encontrada = None
            for conta in contas:
                if conta["id"] == int(conta_id):
            ...
            # Para anônimos, não há transações no banco (sempre retorna vazio)
            return JsonResponse({
                'sucesso': True,
                'conta': {
                    'id': conta_encontrada["id"],
