...
@Component({
  ...
  template: `
    <section class="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <!-- Card: Saldo atual -->
      <div class="bg-white rounded-2xl px-5 py-4 shadow-sm flex flex-col justify-between">
          ...
          <div class="flex items-center justify-center gap-10">
            ...
            @if (cotacaoDolar; as dolar) {
              <div class="flex flex-col items-center gap-1">
                ...
                <span class="font-semibold text-gray-800">R$ {{ dolar.compra | number:'1.2-2' }}</span>
              </div>
            }
            ...
            @if (cotacaoEuro; as euro) {
              <div class="flex flex-col items-center gap-1">
                ...
                <span class="font-semibold text-gray-800">R$ {{ euro.compra | number:'1.2-2' }}</span>
              </div>
            }

            @if (!cotacaoDolar && !cotacaoEuro) {
              <span class="text-gray-400">Carregando cotações...</span>
            }
...
export class CardsResumoComponent {
  ...
  get transacoesReceita(): number {
    return this.transacoes
      .filter(t => t.tipo === 'RECEITA' && t.estado !== 'PENDENTE')
      .reduce((sum, t) => sum + t.valor, 0);
  }

  get transacoesDespesa(): number {
    return this.transacoes
      .filter(t => t.tipo !== 'RECEITA' && t.estado !== 'PENDENTE')
      .reduce((sum, t) => sum + t.valor, 0);
  }
